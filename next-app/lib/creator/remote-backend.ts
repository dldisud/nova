/* ── Remote Backend (ported from mobile/creator/remoteBackend.ts) ── */
...
function normalizeRemoteHistoryEntries(
  rows: RemoteCreatorHistoryRow[],
): AuthorEpisodeHistoryEntry[] {
  return rows
    .map((row) => ({
      id: row.id,
...
export async function listRemotePenNames(userId: string) {
  try {
    ...
    return data.flatMap((row: { pen_name?: string | null }) =>
      typeof row.pen_name === "string" ? [row.pen_name] : [],
    );
  } catch {
    return [];
  }
...
export async function listRemoteDashboardStats(userId: string) {
  try {
    const supabase = getSupabase();
    const { data, error } = await supabase
...
export async function saveRemoteWorkMeta(
  userId: string,
  workId: string,
  meta: AuthorWorkMeta,
) {
...
async function insertRemoteHistory(
  userId: string,
  draft: EpisodeDraft,
  state: DraftPublicationState,
) {
      ...
      : draft.publicationState === "scheduled" || draft.workflowStep === "scheduled"
        ...
        : draft.publicationState === "published" || draft.publicationState === "updated"
          ...
          : existing?.publicationState === "published" ||
              existing?.publicationState === "updated"
            ? "updated"
