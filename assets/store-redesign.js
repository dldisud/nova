(function () {
  ...
  function buildNovelCard(novel, mode, percentOverride) {
    ...
    const genre = esc((novel.tags && novel.tags[0]) || originLabel(novel.originCountry, novel.isTranslation));
    const saleMark = sale ? "<div class='red-badge'>-" + sale + "% 세일</div>" : "";

    return "<a href='" + detailHref(novel.slug) + "' class='spotify-card'>" +
      "<div class='spotify-card-img-wrap'>" +
        saleMark +
        "<img class='spotify-card-cover' src='" + esc(cover(novel)) + "' alt='" + esc(novel.title) + " 표지'>" +
      "</div>" +
      "<div class='spotify-card-title'>" + esc(novel.title) + "</div>" +
      "<div class='spotify-card-sub'>" + genre + " · " + esc(novel.authorName) + "</div>" +
    "</a>";
  }
  ...
  function renderLibrary(data) {
    ...
    var loginPrompt = "<div style='text-align:center; padding:48px 24px; color:#b3b3b3; grid-column:1/-1;'><h3 style='font-size:1rem; color:#fff; margin:0 0 8px;'>" + t("library.login_required") + "</h3><p style='margin:0 0 16px; font-size:0.85rem;'>" + t("library.login_subtitle") + "</p><a class='button primary' href='auth_pc.html'>로그인</a></div>";

    if (readingList) {
      readingList.innerHTML = isLoggedIn
        ? "<div style='text-align:center; padding:48px 24px; color:#b3b3b3; grid-column:1/-1;'><h3 style='font-size:1rem; color:#fff; margin:0 0 8px;'>" + t("library.no_reading") + "</h3><p style='margin:0 0 16px; font-size:0.85rem;'>" + t("library.no_reading_hint") + "</p><a class='button primary' href='search_pc.html'>작품 탐색하기</a></div>"
        : loginPrompt;
    }
    ...
    if (wishlistList) {
      if (bookmarkedNovels.length) {
        wishlistList.innerHTML = bookmarkedNovels.map(function (novel) {
          return buildNovelCard(novel, "free");
        }).join("");
      } else {
        wishlistList.innerHTML = "<div style='text-align:center; padding:48px 24px; color:#b3b3b3; grid-column:1/-1;'><h3 style='font-size:1rem; color:#fff; margin:0 0 8px;'>" + t("library.no_bookmarks") + "</h3><p style='margin:0 0 16px; font-size:0.85rem;'>" + t("library.no_bookmarks_hint") + "</p><a class='button primary' href='search_pc.html'>작품 탐색하기</a></div>";
      }
    }

    if (purchasedList) {
      purchasedList.innerHTML = isLoggedIn
        ? "<div style='text-align:center; padding:48px 24px; color:#b3b3b3; grid-column:1/-1;'><h3 style='font-size:1rem; color:#fff; margin:0 0 8px;'>" + t("library.no_purchases") + "</h3><p style='margin:0 0 16px; font-size:0.85rem;'>" + t("library.no_purchases_hint") + "</p><a class='button primary' href='search_pc.html'>작품 탐색하기</a></div>"
        : loginPrompt;
    }
  ...
  function buildMobileListRow(novel, index) {
    var sale = salePercent(novel);
    var saleMark = sale ? "<div class='red-badge-masonry'>-" + sale + "%</div>" : "";
    var shapes = ["tall", "short", "medium"];
    var shape = shapes[(index || 0) % 3];

    return "<a class='masonry-item " + shape + "' href='" + mobileDetailHref(novel.slug) + "' style='text-decoration:none; display:block;'>" +
      saleMark +
      "<img class='masonry-cover' src='" + esc(cover(novel)) + "' alt='" + esc(novel.title) + "'>" +
      "<div class='masonry-info'>" +
      "<div class='masonry-title'>" + esc(novel.title) + "</div>" +
      "<div class='masonry-author'>" + esc(novel.authorName) + "</div>" +
      "</div>" +
      "</a>";
  }
  ...
  function renderMobileLibrary(data) {
    ...
    var loginPrompt = "<div style='text-align:center; padding:48px 24px; color:#b3b3b3; grid-column:1/-1;'><h3 style='font-size:1rem; color:#fff; margin:0 0 8px;'>" + t("library.login_required") + "</h3><p style='margin:0 0 16px; font-size:0.85rem;'>로그인 후 확인할 수 있습니다.</p><a class='button primary' href='auth_pc.html'>로그인</a></div>";

    if (readingList) {
      readingList.innerHTML = isLoggedIn
        ? "<div style='text-align:center; padding:48px 24px; color:#b3b3b3; grid-column:1/-1;'><h3 style='font-size:1rem; color:#fff; margin:0 0 8px;'>" + t("library.no_reading") + "</h3><p style='margin:0 0 16px; font-size:0.85rem;'>스토어에서 작품을 골라 읽어보세요.</p><a class='button primary' href='search.html'>작품 탐색하기</a></div>"
        : loginPrompt;
    }

    if (wishlistList) {
      if (bookmarkedNovels.length) {
        wishlistList.innerHTML = bookmarkedNovels.map(buildMobileListRow).join("");
      } else {
        wishlistList.innerHTML = "<div style='text-align:center; padding:48px 24px; color:#b3b3b3; grid-column:1/-1;'><h3 style='font-size:1rem; color:#fff; margin:0 0 8px;'>" + t("library.no_bookmarks") + "</h3><p style='margin:0 0 16px; font-size:0.85rem;'>" + t("library.no_bookmarks_hint") + "</p><a class='button primary' href='search.html'>탐색하기</a></div>";
      }
    }

    if (purchasedList) {
      purchasedList.innerHTML = isLoggedIn
        ? "<div style='text-align:center; padding:48px 24px; color:#b3b3b3; grid-column:1/-1;'><h3 style='font-size:1rem; color:#fff; margin:0 0 8px;'>" + t("library.no_purchases") + "</h3><p style='margin:0 0 16px; font-size:0.85rem;'>" + t("library.no_purchases_hint") + "</p><a class='button primary' href='search.html'>탐색하기</a></div>"
        : loginPrompt;
    }
