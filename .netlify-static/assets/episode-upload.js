(function () {
  ...
  if (page !== "episode_upload_pc") return;
  ...
  function currentRelativePath() {
    const name = window.location.pathname.split("/").pop() || "episode_upload_pc.html";
  ...
  function redirectToAuth(mode) {
    if (state.authRedirectPending) return;
  ...
  function renderAuthGate(message) {
    const note = message || t("auth.login_required_msg");
    refs.authShell.innerHTML =
      "<div class='auth-card'><div class='auth-head'><span class='eyebrow'>" + esc(t("editor.auth_gate_title")) + "</span><h2 class='auth-title'>로그인 페이지로 이동합니다</h2><p class='auth-text'>회차 작성 화면 안에서는 로그인하지 않고, 별도 인증 화면에서 처리합니다.</p><p class='auth-note'>" + esc(note) + "</p><div class='auth-actions'><a class='button primary' href='" + esc(authHref()) + "'>로그인하러 가기</a><a class='button secondary' href='" + esc(authHref("signup")) + "'>회원가입</a></div></div></div>";
    showForm(false);
    showEmpty("로그인 후 다시 돌아오면 회차 작성 폼이 자동으로 열립니다.");
